...
export const data = `6-9 z: qzzzzxzzfzzzz
2-4 s: xgmsdts
6-7 k: khqpsvk
1-3 k: knkqh
4-11 z: lwnzgzlmpvz
...
.reduce((col, row) => {
    ...
    let [requirements, dirtyLetter, password] = row.split(' ');
    let [minimumPasswordLength, maximumPasswordLength] = requirements.split('-');
    let letter = dirtyLetter[0]

    col.push({
        registerA: parseInt(minimumPasswordLength),
        registerB: parseInt(maximumPasswordLength),
        letter,
        password,
        meetsMinimumRequirement: function () {
            return utils.passwordMeetsMinimumRequirements(this.password, this.letter, this.registerA)
        },
        meetsMaximumRequirement: function () {
            return utils.passwordMeetsMaximumRequirements(this.password, this.letter, this.registerB)
        },
        meetsLengthRequirements: function () {
            return this.meetsMinimumRequirement.call(this) && this.meetsMaximumRequirement.call(this)
        },
        meetsPositionalRequirements: function () {
            // Using XOR (^) and using !! to cast the resulting bit into a boolean.
            return !!(utils.wordContainsLetterAtPosition(this.password, this.letter, this.registerA)
                ^ utils.wordContainsLetterAtPosition(this.password, this.letter, this.registerB))
        }
    })
